...
# On-policy first-visit MC Control, estimates pi = pi_*
epsilon = 0.5
...
for i in range(10000):
    ...
    for t in range(T-1,0,-1):
        S_t, A_t, R_t1 = traj[t]
        G = gamma*G + R_t1
        if not (S_t, A_t) in [ (s,a) for (s,a,_) in [traj[i] for i in range(0,t)]]:
            Returns[S_t, A_t].append(G)
            Q[S_t][A_t] = sum(Returns[S_t, A_t])/len(Returns[S_t, A_t])          
            A_stars = [ a for (a,v) in Q[S_t].items() if v == max(Q[S_t].values())]
            A_star = random.choice(A_stars)
            for a in actions:
                if a == A_star: policy[S_t][a] = 1 - epsilon + epsilon/len(actions)
                else:           policy[S_t][a] = epsilon/len(actions)
