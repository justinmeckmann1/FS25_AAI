...
def plot_img(img, col_map=plt.cm.gray, figure_size=[3, 3], fig_title=''):
    """
    plot a single image
...
def read_data_orig(data_type='FashionMNIST', storage_path='data'):
    """
    reads MNIST or FashionMNIST data to folder 'data' and gives back images and labels
    :param data_type: 'MNIST', 'CIFAR10' or 'FashionMNIST' (<- default)
    :param storage_path: path to store data in (default 'data')
    :return:
    ...
    if data_type == 'MNIST':
        ...
        #do conversion
        training_data.data = torch.from_numpy(np.moveaxis(training_data.data,3,1))
        training_data.targets = torch.tensor(training_data.targets,dtype=torch.int64)        
        
        #do not forget the test data
        test_data.data = torch.from_numpy(np.moveaxis(test_data.data,3,1))
        test_data.targets = torch.tensor(test_data.targets,dtype=torch.int64)
        
    else:
        print('data_type not supported. Choose MNIST, FashionMNIST or CIFAR\n')
...
def create_file_frame(root_folder, categories, sub_folders = ['train', 'test'], shuffle=False):
    """
    create a dict of pandas DataFrame with list of files and labels for all sub_folders
    :param root_folder: root folder with data
    :param categories: list of categories (must correspond to folder names)
    :param sub_folders: data split folder (with same categories)
    :shuffle: whether to shuffle the data
    :return: dict of pandas DataFrame with files and labels
    """
    ...
    for folder in sub_folders:
        image_list = []
        target_list = []
        for index in np.arange(num_categories):
            full_path = os.path.join(root_folder, folder, categories[index])
            image_file_list = os.listdir(full_path)
        
            target_list.append(len(image_file_list))
    
            tmp_list = [os.path.join(folder, categories[index], image_file) for image_file in image_file_list]
            
            image_list += tmp_list
        
        target_cumsum = torch.tensor(target_list).cumsum(0)
        total_num = target_cumsum[-1]
        targets = torch.zeros(total_num, dtype=torch.int64)
        for ind in np.arange(1, len(target_cumsum)):
            targets[target_cumsum[ind-1]:target_cumsum[ind]] = ind

        indices= np.arange(total_num)
        if shuffle:
            #randonly shuffle images
            indices = torch.randperm(total_num)
        
        pdFrame = pd.DataFrame(data={'images': [image_list[ind] for ind in indices], 
                                     'labels': targets[indices]})
        out_path = os.path.join(root_folder, folder + '.csv')
        pdFrame.to_csv(out_path, index=False)

        my_dict[folder] = pdFrame
        
    return my_dict, num_categories
