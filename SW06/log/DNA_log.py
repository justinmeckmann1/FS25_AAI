...
def add_dna_to_log(dna: DNA, debug=False):
    try:
        ...
        #dna.setMaxAccReached(maxAcc=dna.)
...
def add_dna_set_to_log(population_set):
    # Create a List of the actual population
    pop_list = list(population_set)
    for i in range(len(population_set)):
        actual_cnn = pop_list[i]
        add_dna_to_log(dna=actual_cnn)
