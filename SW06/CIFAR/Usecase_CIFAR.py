...
from CIFAR.CIFAR_Architectures import getCIFARReferenceArchitecture_Kaggle
...
def get_usecase_data(batch_size, debug=False):
    ...
    def preprocess_data(image, label):
        if cfg.use_augmentation:
            ...
            # image = keras.image.random_shift(image, 0.1, 0.1)
...
class CIFARUseCase(object):
    def __init__(self, debug=False):
        ...
        self.input_shape = train_dataset.element_spec[0].shape[1:4]

    def get_reference_model(self, arch):
        if arch == "Kaggle":
            return getCIFARReferenceArchitecture_Kaggle()
        else:
